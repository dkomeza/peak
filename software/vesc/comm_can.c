...
void comm_can_detect_apply_all_foc(uint8_t controller_id, bool activate_status_msgs, float max_power_loss)
{
	...
	comm_can_transmit_eid_replace(controller_id |
																		((uint32_t)CAN_PACKET_DETECT_APPLY_ALL_FOC << 8),
																buffer, send_index, true, 0);
}
															 ...
															 bool store, float start, float end)
{
	...
	comm_can_transmit_eid_replace(controller_id |
																		((uint32_t)(store ? CAN_PACKET_CONF_STORE_BATTERY_CUT : CAN_PACKET_CONF_BATTERY_CUT) << 8),
																buffer, send_index, true, 0);
}
...
void comm_can_shutdown(uint8_t controller_id)
{
	...
	uint8_t buffer[8];
	comm_can_transmit_eid_replace(controller_id |
																		((uint32_t)(CAN_PACKET_SHUTDOWN) << 8),
																buffer, send_index, true, 0);
}
...
static void decode_msg(uint32_t eid, uint8_t *data8, int len, bool is_replaced)
{
	...
	if (id == 255 || id == id1 || id == id2)
	{
		...
		case CAN_PACKET_FILL_RX_BUFFER:
		{
			int buf_ind = -1;
			int offset = data8[0];
			data8++; // Skip offset byte
			len--;	 // Skip offset byte
		...
		{
			ind = 0;
			unsigned int last_id = data8[ind++]; // 0
			commands_send = data8[ind++];				 // 1
			...
			int rxbuf_len = (int)data8[ind++] << 8; // 2
			rxbuf_len |= (int)data8[ind++];					// 3
			...
			if (crc16(rx_buffer[buf_ind], rxbuf_len) == ((unsigned short)crc_high << 8 | (unsigned short)crc_low))
			{
		...
		case CAN_PACKET_POLL_ROTOR_POS:
		{
			...
			comm_can_transmit_eid_replace(app_get_configuration()->controller_id |
																				((uint32_t)CAN_PACKET_POLL_ROTOR_POS << 8),
																		(uint8_t *)buffer, 4, true, 0);
		}
		break;
