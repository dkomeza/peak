...
def make_packet(payload: bytes) -> bytes:
    ...
    if length < 256:
        ...
        header = bytes([3]) + length.to_bytes(2, 'big')
    else:
        header = bytes([4]) + length.to_bytes(3, 'big')

    crc = crc16_ccitt(payload)
    packet = header + payload + crc.to_bytes(2, 'big') + bytes([0x03])
    return packet
...
def main():
    HOST = 'PEAK.local'  # or use '192.168.4.1' or whatever your ESP32 IP is
    PORT = 65102

    # Example: COMM_GET_VALUES = 4 (no payload)
    payload = bytes([0])  # command ID 4 is "Get Values"
    packet = make_packet(payload)

    print("Connecting to ESP32 TCP bridge...")
